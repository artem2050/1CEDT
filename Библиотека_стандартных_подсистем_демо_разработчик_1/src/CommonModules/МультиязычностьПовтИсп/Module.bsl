...
Функция ОбъектНеСодержитТЧПредставления(Ссылка, ИмяРеквизита = "") Экспорт
	
	ТабличныеЧасти = Ссылка.Метаданные().ТабличныеЧасти;
	
	Результат = Ложь;
	Если ТабличныеЧасти.Найти("Представления") = Неопределено Тогда
		Результат = Истина;
	ИначеЕсли ЗначениеЗаполнено(ИмяРеквизита) Тогда
		Результат = ТабличныеЧасти.Представления.Реквизиты.Найти(ИмяРеквизита) = Неопределено;
	КонецЕсли;
