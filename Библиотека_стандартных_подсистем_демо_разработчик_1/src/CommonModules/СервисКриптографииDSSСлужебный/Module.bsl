...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		...
		СтрокаПараметра.Описание = НСтр("ru = 'Сервер электронной подписи DSS'", КодЯзыка());
...
Функция СоздатьУчетнуюЗаписьОблачнойПодписи(НастройкиУчетнойЗаписи) Экспорт
	...
	Если ЕстьИзменения Тогда
		...
		Иначе
			ПротоколОшибкиПриВызове(
				НСтр("ru = 'Не выполнена проверка перед записью учетной записи сервера DSS'", КодЯзыка), 
				"ЗаписьДанныхНастроекУчетнойЗаписи",
				НастройкиУчетнойЗаписи);
		КонецЕсли;
...
Функция ПодготовитьПодключение(СвойстваПодключения, СертификатКлиента = Неопределено) Экспорт
	...
	Попытка
		...
		Если СертификатКлиента = Неопределено 
			...
			СертификатыУЦ = СертификатПодключенияКСервисуИзФайла(СвойстваПодключения.Сервер);
...
Функция ОписаниеОшибки_СертификатБезУчетнойЗаписи()
	...
	Результат = ОбщееОписаниеОшибки("300048", 
			НСтр("ru = 'Выбранный сертификат хранится на сервере DSS, но в параметрах не указана учетная запись DSS'"));
...
Функция СертификатПодключенияКСервисуИзФайла(АдресСервера)
	
	Результат = Новый СертификатыУдостоверяющихЦентровОС;
	
	Если НЕ ОбщегоНазначения.РазделениеВключено()
		ИЛИ ВРег(АдресСервера) <> ВРег("dss.1stdss.1c.ru") Тогда
		Возврат Результат;
	КонецЕсли;
	
	Попытка
		ДанныеФайла = Base64Значение(СертификатыПодключенияСервиса1СDSS());
